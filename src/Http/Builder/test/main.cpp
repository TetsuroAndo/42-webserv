...
static void testDefaultHeaders() {
	...
	ASSERT(out.find("Content-Length: 5") != std::string::npos,
...
static void testUserDefinedHeaders() {
	...
	ASSERT(out.find("Date: CustomDate") != std::string::npos,
		   "Custom Date kept");
	ASSERT(out.find("Server: CustomServer") != std::string::npos,
		   "Custom Server kept");
	ASSERT(out.find("Content-Length: 1234") != std::string::npos,
		   "Custom Content-Length kept");
	ASSERT(out.find("Content-Type: application/json") != std::string::npos,
		   "Custom Content-Type kept");
}
...
static void testBodyOutput() {
	...
	std::string out = ResponseBuilder::build(res);
	ASSERT(out.find("\r\n\r\nHelloWorld") != std::string::npos,
		   "Body correctly placed after headers");
}
...
static void testEmptyBody() {
	...
	res.setBody("");
	std::string out = ResponseBuilder::build(res);
	ASSERT(out.find("Content-Length: 0") != std::string::npos,
		   "Empty body Content-Length 0");
	ASSERT(out.find("\r\n\r\n") != std::string::npos,
		   "Header-body separator exists");
}
...
static void testCRLFFormat() {
	...
	std::string out = ResponseBuilder::build(res);
	size_t pos = out.find("\n");
	ASSERT(pos == std::string::npos || (pos > 0 && out[pos - 1] == '\r'),
		   "CRLF format");
}
