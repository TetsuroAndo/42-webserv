...
static void testBasicHeaders() {
    ...
    ASSERT(req.hasHeader("Host") && req.getHeader("Host") == "example.com", "Host header");
    ASSERT(req.hasHeader("Content-Type") && req.getHeader("Content-Type") == "text/plain", "Content-Type header");
    ASSERT(req.hasHeader("X-Test") && req.getHeader("X-Test") == "123", "X-Test header");
}
...
static void testHeaderWhitespace() {
    ...
    ASSERT(req.hasHeader("Foo-Bar") && req.getHeader("Foo-Bar") == "value", "Whitespace trimmed");
}
...
static void testEmptyHeaderValue() {
    ...
    ASSERT(req.hasHeader("X-Empty") && req.getHeader("X-Empty").empty(), "Empty value stored");
}
