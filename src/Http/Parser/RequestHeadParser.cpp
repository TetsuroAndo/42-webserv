...
ParseResult RequestHeadParser::parse(HttpRequest &request,
	...
	while (lineStart < size) {
		...
		if (lineEnd > lineStart) {
			...
			while (keyStart < keyEnd && (headerBlock[keyStart] == ' ' ||
										 headerBlock[keyStart] == '\t'))
				keyStart++;
			while (keyEnd > keyStart && (headerBlock[keyEnd - 1] == ' ' ||
										 headerBlock[keyEnd - 1] == '\t'))
				keyEnd--;
			...
			while (valueStart < valueEnd && (headerBlock[valueStart] == ' ' ||
											 headerBlock[valueStart] == '\t'))
				valueStart++;
			while (valueEnd > valueStart && (headerBlock[valueEnd - 1] == ' ' ||
											 headerBlock[valueEnd - 1] == '\t'))
				valueEnd--;
