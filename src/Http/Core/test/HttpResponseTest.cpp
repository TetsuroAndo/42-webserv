...
static void testConstructorAndClear() {
	...
	std::cout << (res.getServerName() == "TestServer" ? PASS : FAIL)
			  << " ServerName default\n";
	std::cout << (res.getStatusCode() == 200 ? PASS : FAIL)
			  << " Status default\n";
	std::cout << (res.getVersion() == HTTP_VERSION ? PASS : FAIL)
			  << " Version default\n";
	...
	std::cout << (res.getStatusCode() == 200 ? PASS : FAIL)
			  << " Status after clear\n";
	std::cout << (res.getHeaders().empty() ? PASS : FAIL)
			  << " Headers after clear\n";
...
static void testServerName() {
	...
	std::cout << (res.getServerName() == "MyServer" ? PASS : FAIL)
			  << " ServerName set/get\n";
}
...
static void testStatusCode() {
	HttpResponse res("TestServer");
	res.setStatusCode(404);
	std::cout << (res.getStatusCode() == 404 ? PASS : FAIL)
			  << " StatusCode set/get\n";
}
...
static void testVersion() {
	HttpResponse res("TestServer");
	res.setVersion("HTTP/1.1");
	std::cout << (res.getVersion() == "HTTP/1.1" ? PASS : FAIL)
			  << " Version set/get\n";
}
...
static void testHeaders() {
	...
	std::cout << (res.hasHeader("Content-Type") ? PASS : FAIL)
			  << " hasHeader\n";
	std::cout << (res.getHeader("Content-Type") == "text/html" ? PASS : FAIL)
			  << " getHeader\n";
}
