...
static void assertEqual(const std::string &name,
    ...
    if (!ok) {
        ...
        for (std::map<std::string, std::string>::const_iterator it = expect.begin(); it != expect.end(); ++it) {
            std::cerr << "    {" << it->first << ": '" << it->second << "'}\n";
        }
        std::cerr << "  Got:" << std::endl;
        for (std::map<std::string, std::string>::const_iterator it = got.begin(); it != got.end(); ++it) {
            std::cerr << "    {" << it->first << ": '" << it->second << "'}\n";
        }
        ++failures;
    } else {
...
int main() {
    ...
    cases.back().expect["p"] = "pathtofile"; // unescapeCookieValue removes escapes

    // Quoted value that includes escaped semicolon (treated as normal char due to unescape) and comma
    cases.push_back(TestCase());
    ...
    for (size_t i = 0; i < cases.size(); ++i) {
        const std::map<std::string, std::string> got = parseCookieField(cases[i].input);
