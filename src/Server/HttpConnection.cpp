...
HttpConnection::HttpConnection(Client* client, PipelineContext* context, HttpConnectionEventHandler* eventHandler)
...
void HttpConnection::handleReadEvent() {
    ...
    if (bytesRead > 0) {
        ...
        LOG(INFO) << "Client disconnected gracefully" << attr("fd", _client->getFd());
        if (_eventHandler) {
...
void HttpConnection::handleWriteEvent() {
    ...
    const ssize_t bytesSent = send(_client->getFd(), sendBuffer.c_str(), sendBuffer.size(), 0);
...
void HttpConnection::generateResponse() {
    if (_context->parser.isComplete() || _context->parser.getErrorCode() != 0) {
        ...
        if (!responseStr.empty()) {
            _client->getSocket()->setSendBuffer(_client->getSocket()->getSendBuffer() + responseStr);
        }
