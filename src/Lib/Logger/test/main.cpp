...
void testErrorLogger() {
	...
	logger.setSinkFile("error_elf.log", ELF, INFO, GREATER_OR_EQUAL,
					   1024 * 1024, 5);

	// 3. ファイル出力 (JSON形式, WARNINGレベル以上)
	logger.setSinkFile("error_json_warn.log", JSON, WARNING, GREATER_OR_EQUAL,
					   1024 * 1024, 5);

	// 4. ファイル出力 (ELF形式, ERRORレベルのみ)
	logger.setSinkFile("error_elf_exact.log", ELF, ERROR, EXACT, 1024 * 1024,
					   5);
	...
	LOG(DEBUG) << "This is a debug message. Should appear on console and "
				  "rotation.log.";

	LOG(INFO) << "Server started successfully." << attr("host", "localhost")
			  << attr("port", 8080);
	...
	std::cout
		<< "Rotation test messages sent. Check for rotation.log.1, .2, etc."
		<< std::endl;
}
...
void testAccessLogger() {
	...
	// --- Test Case 2: クエリ付きPOSTリクエスト (404 Not Found) ---
	{
		...
		accessLogger.log(&req, &res, "10.0.0.5", 12345,
						 ""); // セッションIDなし
	}
}
...
int main() {
	try {
		...
		std::cout << "Please check the console output and the files in "
					 "'./test_log/' directory:"
				  << std::endl;
		...
		std::cout << "  - rotation.log, rotation.log.1, ... (Log rotation test)"
				  << std::endl;
		std::cout << "  - access_elf.log (Access log in ELF format)"
				  << std::endl;
		std::cout << "  - access_json.log (Access log in JSON)" << std::endl;
		std::cout << "\nNOTE: Access logs in ELF format are also printed to "
					 "the console."
				  << std::endl;
		std::cout << "========================================================"
				  << std::endl;
		...
		std::cerr << "\n[FATAL] A critical error occurred during testing: "
				  << e.what() << std::endl;
		return 1;
	}
	return 0;
