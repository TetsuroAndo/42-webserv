...
void test_normalize() {
	...
	// 基本ケース
	path = "/a/b/c";
	ASSERT_EQ("/a/b/c", Path::normalize(path));
	path = "/a/b/../c";
	ASSERT_EQ("/a/c", Path::normalize(path));
	path = "/a/./b/c/";
	ASSERT_EQ("/a/b/c", Path::normalize(path));
	path = "//a//b";
	ASSERT_EQ("/a/b", Path::normalize(path));
	...
	ASSERT_EQ(".", Path::normalize(path));
	path = "..";
	ASSERT_EQ("..", Path::normalize(path)); // 相対パスの".."はそのまま
	path = "/..";
	ASSERT_EQ("/", Path::normalize(path));
	path = "/a/../../b";
	ASSERT_EQ("/b", Path::normalize(path));
	path = "a/../b";
	ASSERT_EQ("b", Path::normalize(path));
	path = "";
	ASSERT_EQ(".", Path::normalize(path));
	path = "a/b/../../..";
	ASSERT_EQ("..", Path::normalize(path)); // "a/b" -> "a" -> "" -> ".."
	path = "../../a";
	ASSERT_EQ("../../a", Path::normalize(path));
	path = "/a/b/..";
	ASSERT_EQ("/a", Path::normalize(path));
	path = "a/b/../..";
	ASSERT_EQ(".", Path::normalize(path)); // "a/b" -> "a" -> "" -> "."
	...
	path = "/a/./b/.././../cpop";
	ASSERT_EQ("/cpop", Path::normalize(path));
	...
	ASSERT_EQ("/a", Path::normalize(path));
	path = "a/..";
	ASSERT_EQ(".", Path::normalize(path)); // a -> "" -> "."
...
void test_getAbsolutePath() {
	...
	// 存在しないパスのテスト
	assert(Path::getAbsolutePath("/path/to/non/existent/file") ==
		   ""); // Use direct assert
