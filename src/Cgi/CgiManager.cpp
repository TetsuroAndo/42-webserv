...
void CgiManager::createWorker(PipelineContext &ctx) {
	try {
			...
			if (it != loc.cgiConf.end()) {
				...
				if (!HandlerUtil::extractCgiScript(ctx.req.getPath(), loc,
					...
					HandlerUtil::generateSimpleBody(
						ctx.req.getMethod(), ctx.res, HttpStatus::NOT_FOUND);
					return;
				}
				...
				if (interpreterPath.empty()) {
					LOG(WARNING)
						<< "No CGI interpreter found for the request path: "
						<< scriptPath;
					HandlerUtil::generateSimpleBody(
						ctx.req.getMethod(), ctx.res, HttpStatus::NOT_FOUND);
					return;
				...
				try {
					...
					delete worker;
					LOG(ERROR) << "Failed to create or execute CgiWorker: "
							   << e.what();
					HandlerUtil::generateSimpleBody(
						ctx.req.getMethod(), ctx.res,
		...
		void CgiManager::cleanupTimedOutWorkers() {
			...
			for (std::vector< CgiWorker * >::iterator it =
					 workersToCleanup.begin();
		...
		void CgiManager::cleanupFinishedWorkers() {
			...
			while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
				...
				CgiWorker *worker = NULL;
				std::map< pid_t, CgiWorker * >::iterator pit =
					_pidToWorker.find(pid);
				...
				if (worker) {
					...
					// が完了したプロセス。
					LOG(DEBUG)
						<< "Reaped zombie process (PID not in active workers "
