...
namespace {
    ...
    void parseLogs(const Node *node, const std::string &logKey, const std::set<std::string>& enabledValidKeys, std::vector<LogType> &configuredLogs) {
        if (!node) return;
        ...
        for (std::vector<Node *>::const_iterator it = logs.begin(); it != logs.end(); ++it) {
            ...
            if (isDisabled) {
                ...
                ConfigParser::validateKeys(logNode, (logKey == "access_log" ? ConfigParser::VALID_DISABLED_ACCESS_LOG_KEYS : ConfigParser::VALID_DISABLED_ERROR_LOG_KEYS), "disabled " + logKey + " block");
                log.isDisable = true;
            ...
            isEnabledFound = true;
            ConfigParser::validateKeys(logNode, enabledValidKeys, logKey + " block");
...
void ConfigLogParser::parseAccessLogs(const Node *node) {
    ...
    parseLogs(node, "access_log", ConfigParser::VALID_ACCESS_LOG_KEYS, configuredLogs);
...
void ConfigLogParser::parseErrorLogs(const Node *node) {
    ...
    parseLogs(node, "error_log", ConfigParser::VALID_ERROR_LOG_KEYS, configuredLogs);
