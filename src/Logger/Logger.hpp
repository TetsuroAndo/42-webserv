...
public:
	...
	void setSinkFile(const std::string &logDir, const std::string &filename,
		LogFormat format, LogLevel level, LogFilterMode mode = GREATER_OR_EQUAL,
		size_t maxFileSize = _LOG_MAX_FILE_SIZE, size_t maxBackupFiles = _LOG_MAX_BACKUPS);
	void setSinkConsole(LogFormat format, LogLevel level, LogFilterMode mode = GREATER_OR_EQUAL);
