...
bool Server::init(const std::string &host, int port) {
	...
	// バインド
	if (bind(_listenFd, (struct sockaddr *)&server_addr, sizeof(server_addr)) <
		0) {
	...
	while (true) {
		...
		for (size_t i = 0; i < _fds.size(); ++i) {
			...
			if (_fds[i].fd == _listenFd && (_fds[i].revents & POLLIN)) {
				...
				int clientFd = accept(
					_listenFd, (struct sockaddr *)&client_addr, &addrlen);
				if (clientFd >= 0) {
					...
					cfd.fd = clientFd;
					cfd.events =
						POLLIN | POLLOUT; // 読み込みと書き込みの両方を監視
					cfd.revents = 0;
					_fds.push_back(cfd);
			...
			// クライアントソケットから読み込み
				...
				if (n <= 0) {
					...
					std::cout
						<< "Connection closed or error. fd = " << _fds[i].fd
						<< std::endl;
					close(_fds[i].fd);
					...
					// 例:
					// _clientStates[fd].appendToReceiveBuffer(received_data);
					...
					std::string file_path =
						"./html/index.html"; // 仮のパス。本来はLocationConfigから決定
					...
					if (ifs.is_open()) {
						...
						std::stringstream
							ss_length; // << Content-Length 用のstringstream
						ss_length
							<< response_body.length(); // response_body.length()
													   // をstringstreamに入れる
						response_header +=
							"Content-Length: " + ss_length.str() +
							"\r\n";				   // 文字列に変換して追加
						response_header += "\r\n"; // ヘッダの終端
						...
						// 例:
						// _clientStates[_fds[i].fd].setResponseToSend(full_response);
						// 現在はデモのために直接send
						send(_fds[i].fd, full_response.c_str(),
							 full_response.size(), 0);
						std::cout << "Sent index.html to fd: " << _fds[i].fd
								  << std::endl;
						...
						std::string not_found_response =
							...
							"\r\n"
							"404 Not Found\n";
						send(_fds[i].fd, not_found_response.c_str(),
							 not_found_response.size(), 0);
						std::cout << "Sent 404 Not Found to fd: " << _fds[i].fd
								  << std::endl;
