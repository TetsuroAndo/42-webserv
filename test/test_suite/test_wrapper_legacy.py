...
TEST_BK_DIR = ".legacy_scripts"


class TestLegacyWrapper:
    """.legacy_scriptsのBashスクリプトをラップするテストクラス"""
    ...
    def test_validation_scripts(self):
        """.legacy_scripts/validation_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"validation_test/run.sh が失敗しました (exit code: {result.returncode})"
        )

    def test_static_file_scripts(self):
        """.legacy_scripts/static_file_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"static_file_test/run.sh が失敗しました (exit code: {result.returncode})"
        )

    def test_cgi_scripts(self):
        """.legacy_scripts/cgi_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"cgi_test/run.sh が失敗しました (exit code: {result.returncode})"
        )

    def test_post_scripts(self):
        """.legacy_scripts/post_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"post_test/run.sh が失敗しました (exit code: {result.returncode})"
        )

    def test_redirect_scripts(self):
        """.legacy_scripts/redirect_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"redirect_test/run.sh が失敗しました (exit code: {result.returncode})"
        )

    @pytest.mark.skip(reason="セッションテストは実装中")
    def test_session_scripts(self):
        """.legacy_scripts/session_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"session_test/run.sh が失敗しました (exit code: {result.returncode})"
        )

    @pytest.mark.skip(reason="load_testは別途実装予定")
    def test_load_scripts(self):
        """.legacy_scripts/load_test/run.sh を実行"""
        ...
        assert result.returncode == 0, (
            f"load_test/run.sh が失敗しました (exit code: {result.returncode})"
        )
