...
class TestGET:
    ...
    @pytest.mark.config("valid/config_autoindex_on.yaml")
    def test_autoindex_on_with_no_index_true(self, managed_server):
        url = f"{managed_server['base_url']}/without_index"
        response = requests.get(url)
        assert response.status_code == 200
        assert "<html><head><title>Index of " in response.text
    ...
    @pytest.mark.config("valid/config_autoindex_off.yaml")
    def test_autoindex_off_and_no_index_true(self, managed_server):
        url = f"{managed_server['base_url']}/with_noIndex"
        response = requests.get(url)
        assert response.status_code == 403
