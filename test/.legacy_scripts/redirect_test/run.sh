...
run_test "test/confs/valid/config_301.yaml" "Basic 301" "/old-path" "301 Moved Permanently" "/new-path" || exit 1

# 2. Basic 302 Redirect
run_test "test/confs/valid/config_302.yaml" "Basic 302" "/temp-old" "302 Found" "/temp-new" || exit 1

# 3. Prefix Matching (Longest Match)
#    - Test /prefix/path/resource should match /prefix/path
run_test "test/confs/valid/config_prefix.yaml" "Prefix Longest Match" "/prefix/path/resource" "301 Moved Permanently" "/new-prefix/path-specific/resource" || exit 1
#    - Test /prefix/resource should match /prefix
run_test "test/confs/valid/config_prefix.yaml" "Prefix Shorter Match" "/prefix/resource" "301 Moved Permanently" "/new-prefix/resource" || exit 1
...
run_test "test/confs/valid/config_301.yaml" "No Redirect" "/non-existent-path" "404 Not Found" "" || exit 1

# 5. External Redirect
run_test "test/confs/valid/config_external.yaml" "External Redirect" "/external" "302 Found" "http://example.com" || exit 1

# 6. Redirect with Query Parameters
run_test "test/confs/valid/config_query.yaml" "Query Params Redirect" "/query?param=value&another=test" "307 Temporary Redirect" "/new-query?param=value&another=test" || exit 1
