...
ERROR_OUTPUT=$($WEBSERV_BIN test/confs/invalid/config_invalid_method.yaml 2>&1)
...
run_curl_test "No allowedMethods (GET)" \
              "test/confs/valid/config_no_methods.yaml" \
              "http://localhost:8080/" \
              "200"
...
run_curl_test "Disallowed Method (POST to GET-only)" \
              "test/confs/valid/config_get_only.yaml" \
...
run_curl_test "Multiple Locations (GET on /)" \
              "test/confs/valid/config_multiple_locations.yaml" \
              "http://localhost:8080/" \
              "200"
