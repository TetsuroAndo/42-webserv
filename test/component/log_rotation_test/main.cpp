...
#include "../../src/Config/ConfigBuilder.hpp"
...
void setupLogger(const Config &config) {
	try {
		...
		if (!accessLogs.empty()) {
			...
			AccessLogger::getInstance().setSinkFile(
				logConf.logDir, logConf.filename, logConf.format,
				logConf.maxFileSize, logConf.maxBackupFiles);
		}
	} catch (const std::exception &e) {
		std::cerr << "Logger Setup Error: " << e.what() << std::endl;
		exit(1);
	}
}
...
int main(int argc, char **argv) {
	...
	Config config = ConfigBuilder(argv[1]).build();
	...
	for (int i = 0; i < 12; ++i) {
		AccessLogger::getInstance().log(&req, &res, "127.0.0.1", 12345,
										"session-id-12345");
	}
